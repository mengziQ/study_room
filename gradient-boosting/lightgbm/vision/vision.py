...
if '--scan' in sys.argv:
  names = [name for name in glob.glob('/home/irep/mnt_s3/outputs/i2v_ad/minimized/*.png')]
...
if '--minimize' in sys.argv:
  ...
  def _minimize(name):
    try:
      ...
      save_name = '/home/irep/mnt_s3/outputs/i2v_ad/minimized/' + name.split('/').pop() + '.png'
      try:
        print('画像開けます！')
        img = Image.open(name)
      except OSError as e:
  ...
  names = [name for name in glob.glob('/home/irep/mnt_s3/outputs/i2v_ad/imgs/*')]
